        ...
        if post.get("status") is True:
            try:
                date_obj = datetime.strptime(
                    post["created_at"].split("T")[0], "%Y-%m-%d"
                )
            ...
            post["excerpt"] = (
                raw_content.replace("<p>", "").replace("</p>", "")[:120] + "..."
            )
...
def blog_post_detail(blog_id):
    ...
    post = next(
        (
            p
            for p in posts
            if str(p.get("id")) == str(blog_id) and p.get("status") is True
        ),
        None,
    )
    if not post:
    ...
    related_posts = [
        p for p in posts if str(p.get("id")) != str(blog_id) and p.get("status") is True
    ][:3]
    for rel in related_posts:
        ...
        rel["excerpt"] = (
            rel.get("content", "").replace("<p>", "").replace("</p>", "")[:100] + "..."
        )
